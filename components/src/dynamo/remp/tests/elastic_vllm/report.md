...
使用 `service.sh sync` 命令（需设置 `ELASTIC_VLLM_GITHUB_TOKEN`）执行自动同步：

```bash
export ELASTIC_VLLM_GITHUB_TOKEN=<your-token>
cd /workspace
bash /workspace/dynamo/components/src/dynamo/remp/tests/elastic_vllm/service.sh sync
```

> **注意：** 由于 `service.sh` 中的路径解析基于脚本原始位置（`<workspace>/dynamo/recipes/elastic-vllm/service.sh`），当前脚本已被迁移至 `<workspace>/dynamo/components/src/dynamo/remp/tests/elastic_vllm/service.sh`，路径解析的 `WORKSPACE_DIR` 会错误地指向 `/workspace/dynamo/components/src/dynamo` 而非 `/workspace`。本次测试采用手动同步方式：
...
# 1. Git clone ElasticVllm_demo（私有仓库，需 token）
cd /workspace
git clone --depth 1 -b codex/add-v0.22.0 \
  https://<token>@github.com/yhp49/ElasticVllm_demo.git ElasticVllm_demo

# 2. dynamo 仓库已存在（ElasticVllm 分支），git pull
cd /workspace/dynamo && git pull

# 3. 复制 ElasticVllm_demo/vllm（弹性 vLLM 引擎核心代码）
cp -rf /workspace/ElasticVllm_demo/vllm/* "${VLLM_SITE}/"

# 4. 复制 dynamo/components/src/dynamo/vllm（Dynamo vLLM 后端适配层）
cp -rf /workspace/dynamo/components/src/dynamo/vllm/* "${DYNAMO_VLLM_SITE}/"

# 5. 复制 dynamo/components/src/dynamo/remp（REMP 模块）
cp -rf /workspace/dynamo/components/src/dynamo/remp "${DYNAMO_SITE}/remp"

# 6. 仅复制 dynamo/common/constants.py（新增 KV_HINT_TRANSFER_CAPABILITY_KEY）
...
# 7. 应用全部本地补丁
PATCH_DIR="/workspace/dynamo/components/src/dynamo/remp/tests/elastic_vllm/patches"
...
python3 -c "from vllm.exceptions import VLLMClientError, VLLMNotFoundError, VLLMUnprocessableEntityError; print('✅ OK')"
```

### 2.2 补丁清单

同步过程中对 `site-packages` 中的文件进行修补，均已整理为标准 unified diff 格式的
patch 文件，存放于 `dynamo/components/src/dynamo/remp/tests/elastic_vllm/patches/` 目录下。

| # | 补丁文件 | 目标文件 | 分类 | 用途 |
|---|----------|----------|------|------|
| 1 | `vllm.patch` | `vllm/exceptions.py` | 缺失依赖 | 新增 `VLLMClientError`、`VLLMUnprocessableEntityError` 异常类 |
| 2 | `vllm.patch` | `vllm/model_executor/layers/fla/ops/chunk_delta_h.py` | 硬件适配 | MetaX C550 共享内存限制：`num_stages` 需为 1 |
| 3 | `vllm.patch` | `vllm/v1/engine/async_llm.py` | Bug 修复 | 切换后同步 `vllm_config.parallel_config` |
| 4 | `dynamo.patch` | `dynamo/common/utils/runtime.py` | API 兼容 | 为 `create_runtime()` 添加 `response_plane` 参数占位 |
| 5 | `dynamo.patch` | `dynamo/vllm/main.py` | API 兼容 | `config.response_plane` → `getattr` 安全访问 |
| 6 | `dynamo_common_constants_add_kv_hint.patch` | `dynamo/common/constants.py` | 缺失依赖 | 新增 KV_HINT_TRANSFER_CAPABILITY_KEY 等常量 |

<details>
<summary>补丁详细说明</summary>

#### ① vllm.patch — exceptions.py 部分（缺失依赖）
...
#### ② vllm.patch — chunk_delta_h.py 部分（硬件适配）
...
#### ③ vllm.patch — async_llm.py 部分（Bug 修复）
...
#### ④ dynamo.patch — runtime.py 部分（API 兼容）
...
#### ⑤ dynamo.patch — main.py 部分（API 兼容）
...
#### ⑥ dynamo_common_constants_add_kv_hint.patch（缺失依赖）

- **修改文件**：`dynamo/common/constants.py`
- **原因**：新版 `dynamo/vllm` 代码引用了 `KV_HINT_TRANSFER_CAPABILITY_KEY` 等常量，但旧版 pip 安装的 `dynamo/common/constants.py` 不包含这些常量
- **修改内容**：新增 `KV_HINT_TRANSFER_CAPABILITY_KEY`、`KV_HINT_TRANSFER_WORKER_TYPE_RUNTIME_KEY`、`KV_HINT_TRANSFER_SOURCE_CONTROL_ENDPOINTS_RUNTIME_KEY` 常量定义
...
> **备注：** `dynamo.patch` 中的 `runtime.py` 部分因代码已包含 `response_plane` 参数（由最新版本代码自带），patch 工具无法匹配但实际内容已满足要求，不影响功能。

### 2.4 `service.sh` 适配（InfiniCore 模式）

为支持 InfiniCore 平台插件，`service.sh` 已包含以下配置：

1. **`VLLM_PLUGINS` 设为 `infinicore`**
2. **环境变量**：`VLLM_INFINICORE_GDN_SINGLE_STAGE=1`、`MACA_PATH`/`MACA_HOME`/`MACA_ROOT`
3. **`FLASH_ATTN_2_CUDA_SO` 动态解析**：使用 Python `importlib.util.find_spec()` 在运行时定位 `flash_attn_2_cuda` 的 `.so` 文件路径
4. **默认模型路径为 `Qwen3.8-27B`**
...
3. **补丁与上游版本耦合**：补丁 ①②③ 针对 ElasticVllm_demo 的 `codex/add-v0.22.0` 分支生成，补丁 ④⑤ 针对 dynamo 的 `ElasticVllm` 分支生成，补丁 ⑥ 针对 dynamo 的 `ElasticVllm` 分支生成。若上游代码变更，需重新生成补丁。
...
python3 /workspace/dynamo/components/src/dynamo/remp/tests/elastic_vllm/test_offline_switch.py \
...
export MACA_ROOT=$MACA_PATH
export FLASH_ATTN_2_CUDA_SO=/opt/conda/lib/python3.10/site-packages/flash_attn_2_cuda.cpython-310-x86_64-linux-gnu.so
export DYN_SYSTEM_PORT=9091

# 启动 backend
nohup python -m dynamo.remp \
...
# 启动 frontend
nohup python -m dynamo.frontend \
  --discovery-backend file \
  --http-port 9090 \
  >> /workspace/logs/frontend.log 2>&1 &

# 运行在线切换测试
python3 /workspace/dynamo/components/src/dynamo/remp/tests/elastic_vllm/test_online_switch_dynamo.py
```
...
5. **权重重载耗时**：27B 模型每次切换后权重重载约 18s ~ 27s，主要由磁盘 I/O（18-shard safetensors）和模型加载决定。在线模式下 2×2→1×4 的切换耗时较长（33.3s），因 PP4 需要串行加载更多 pipeline stage 权重。
...
8. **补丁批量应用**：所有补丁均为标准 unified diff 格式，支持正向应用与反向还原。补丁清单详见 §2.2。
9. **service.sh 路径迁移**：`service.sh` 已从 `dynamo/recipes/elastic-vllm/` 迁移至 `dynamo/components/src/dynamo/remp/tests/elastic_vllm/`，其中 `WORKSPACE_DIR` 的路径计算逻辑需要相应调整（`../../..` → `../../../../..`），否则 sync 命令会定位到错误的目录。
