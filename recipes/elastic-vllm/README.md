...
# vLLM 原生模式（后台）
./dynamo/recipes/elastic-vllm/service.sh vllm --background --gpu-memory-utilization 0.4 --enforce-eager

# Dynamo 模式（后台，含 backend + frontend）
./dynamo/recipes/elastic-vllm/service.sh dynamo --background --gpu-memory-utilization 0.4 --enforce-eager
```

> **重要**：在线切换必须加 `--enforce-eager`，否则切换时 Worker 会因 CUDA Graph 编译卡住导致 RPC 超时。
...
# 查询当前并行策略状态
./dynamo/recipes/elastic-vllm/service.sh status
```

### 5. 手动触发切换

```bash
./dynamo/recipes/elastic-vllm/service.sh switch \
    --new_world_size 4 \
